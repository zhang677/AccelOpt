...
def get_baseline_latency(baseline_df: pd.DataFrame, case_name: str) -> float:
    row = baseline_df[baseline_df["case_name"] == case_name]
    if row.empty:
        raise RuntimeError(f"Baseline latency not found for case_name={case_name}")
    return float(json.loads(row["profile"].iloc[0])["latency"])
...
def get_latency_dict(record: dict, max_plans: int = None, max_sample_id: int = None) -> Dict[Tuple[str, str], Dict[str, Any]]:
    ...
    for k, v in record.items():
        if isinstance(v, dict) and "latency" in v and v["latency"] is not None:
            if "bmm_softmax" in service_name and not ("nl.max" in v["body"] or "np.max" in v["body"]): # Filter out the wrong code
                continue
            ...
            d[(service_name, k)] = {
                "latency": v["latency"],
                "kernel_metadata": v["kernel_metadata"],
                "plan_id": k,
...
def calc_best_metadata_for_file(executor_results_json: str, baseline_latency: float, 
    ...
    for item_key, lat in complete_lat_dict.items():
        if lat["latency"] > 0:
            ...
            if sp > best:
                best = sp
                best_metadata = lat["kernel_metadata"]
