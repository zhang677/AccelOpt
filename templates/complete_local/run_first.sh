...
python $CONSTRUCT_EXPERIENCE_EXEC --is_first --experience_list_path $EXPERIENCE_LIST_PATH --output_path $CONSTRUCT_EXPERIENCE_OUTPUT_PATH --n $EXP_N
...
PLANNER_EXEC="$ACCELOPT_BASE_DIR/scripts/planner.py"
...
python $PLANNER_EXEC --output_path $PLANNER_OUTPUT_PATH --breadth $BREADTH --exp_dir $EXP_DIR --base_prompt_path $PLANNER_PROMPT_CONSTRUCTOR_NEW_BASE_PROMPT_PATH --user_template_path $PLANNER_USER_TEMPLATE_PATH --profile_result_path $PLANNER_PROFILE_RESULT_PATH --model_config_path $PLANNER_MODEL_CONFIG_PATH --displayed_profiles_path $PLANNER_DISPLAYED_PROFILES_PATH
