...
def construct_user_prompt(user_prompt_config: UserPromptConfig):
    ...
    user_prompt = user_prompt_config.prompt_template.replace("{definition_str}", format_definition(definition))
...
def separate_reasoning(response: Optional[ChatCompletion]):
    if response is None:
        return None, None
    plan = response.choices[0].message.content
    # Handles reasoning field for models like DeepSeek or OpenAI o1
    reasoning = getattr(response.choices[0].message, 'reasoning', plan)
    return reasoning, plan

async def single_query(single_record, client: AsyncOpenAI, config: UserPromptConfig):
    ...
    tasks = [retry_query_coroutine(q_co, max_retries=15, delay=10) for _ in range(config.breadth)]
    ...
    for result in results:
        reasoning, plan = separate_reasoning(result)
...
async def main(kwargs):
    ...
    return await single_query(record_data, client, user_prompt_config)
