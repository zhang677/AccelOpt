...
def construct_summarizer_prompt(config: SummarizerPromptConfig):
    prompt = config.prompt_template.replace("{slow_kernel}", config.slow_kernel)
    prompt = prompt.replace("{fast_kernel}", config.fast_kernel)
    return prompt

async def sample_once(config: SummarizerPromptConfig, client, name):
    try:
        ...
        if response is None:
            logger.info(f"[Skip] Failed to get result for {name}")
            return
        return response
    except Exception as e:
        logger.info(f"[Error] Failed to process {name}: {e}")
        traceback.print_exc()
        raise

async def process_optimization_item(executor_result, k, v, client, config: SummarizerPromptConfig):
    """Process a single optimization item summarizer"""
    service_name = executor_result["baseline_solution_path"]
    case_name = executor_result["workload_path"]
    
    try:
        # Determine fast and slow kernels based on speedup
        baseline_kernel_body = load_json_file(Solution, v["baseline_solution_path"]).sources[0].content
        proposed_kernel_body = load_json_file(Solution, v["solution_path"]).sources[0].content
        if v["speedup"] < 1.0:
            fast_kernel, slow_kernel = baseline_kernel_body, proposed_kernel_body
        else:
            fast_kernel, slow_kernel = proposed_kernel_body, baseline_kernel_body
        prompt_config = config.model_copy(
            update={"slow_kernel": slow_kernel, "fast_kernel": fast_kernel}
        )
        logfire_service_name = f"summarizer_execution_{load_json_file(Solution, service_name).name}_{k}"
        result = await sample_once(prompt_config, client, logfire_service_name)
        if result is None:
            logger.info(f"[Skip] Failed to get result for {service_name}_{k}")
            return None
        ...
        if "No optimization found" in result.choices[0].message.content:
            optimization_menu_item["title"] = "**No optimization found**"
            return optimization_menu_item
        else:
            optimization_menu_item["summary"] = result.choices[0].message.content
            # Try to extract the title from the summary
            title = re.search(r'\*\*(.*?)\*\*', result.choices[0].message.content)
            optimization_menu_item["title"] = title.group(0) if title else ""
        
        return optimization_menu_item
        
    except Exception as e:
        logger.info(f"[Error] Failed to process {service_name}_{k}: {e}")
        traceback.print_exc()
        return None
...
async def main(args):
    ...
    client = AsyncOpenAI(base_url=summarizer_base_url, api_key=API_KEY, timeout=10000000)
...
# Heuristic starts here:
    ...
    for executor_result in executor_results["executor_results"]:
        ...
        for k in service_name_to_selected_plan_ids[service_name]:
            ...
            task = process_optimization_item(executor_result, k, v, client, prompt_config)
            tasks.append(task)
    
    # Execute all tasks in parallel
    logger.info(f"Processing {len(tasks)} optimization items in parallel...")
    ...
    for result in results:
        if isinstance(result, Exception):
            logger.info(f"[Error] Task failed with exception: {result}")
            continue
    ...
    logger.info(f"Successfully processed {len(optimization_menu_list)} optimization items")
